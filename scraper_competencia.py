...
# =========================================================
# MÉTODO A: inventory_json (LEBUMP)
# =========================================================
...
def extract_inventory_json(base_url: str, product: dict):
    ...
    try:
        ...
        script_tag = soup.select_one('script[data-product-inventory-json]')

        if not script_tag:
            print(f"      inventory_json no encontrado en {handle}")
            return rows
        ...
        for variant_id, val in inventory.items():
            ...
            rows.append({
                ...
                "inventory_message": val.get("inventory_message", ""),
                "available": int(qty > 0),
            })

    except Exception as e:
        print(f"      extract_inventory_json fallo ({handle}): {e}")

    return rows

# =========================================================
# MÉTODO B: data attribute (LABARRIGUITA)
# =========================================================
...
def extract_data_attribute(base_url: str, product: dict):
    ...
    try:
        ...
        for el in elements:
            ...
            rows.append({
                "variant_id": str(el.get("data-variant-id", "")),
                "variant_title": el.get("data-variant-title", ""),
                "sku": el.get("data-sku", ""),
        ...
        print(f"      extract_data_attribute fallo ({handle}): {e}")

    return rows

# =========================================================
# MÉTODO C: variant_available (las demás tiendas)
# =========================================================
...
def scrape_store(store: dict):
    ...
    for i, product in enumerate(products):
        ...
        for r in rows:
            ...
            r["product_name"] = product.get("title", handle)
            r["handle"] = handle
            r["vendor"] = product.get("vendor", "")
            r["product_type"] = product.get("product_type", "")
            r["tags"] = (
